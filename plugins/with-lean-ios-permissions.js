 ...
 *
 * CURB requests When In Use and nothing else. It never asks for background
 ...
 *
 * Verified by inspecting the generated ios/CURB/Info.plist, which is the only
 * way to know: both orderings "succeed".
 */

const { withInfoPlist } = require('expo/config-plugins');

/** Permissions CURB genuinely does not use. */
...
module.exports = function withLeanIosPermissions(config) {
  return withInfoPlist(config, (mod) => {
    ...
    if (reintroduced.length > 0) {
      throw new Error(
        `Info.plist declares background-location permissions CURB does not request: ` +
