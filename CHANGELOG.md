...
- Year, permit, day and time are drop-down menus rather than a button per option. As buttons they came to forty-one, which crowded out the destination field most people open the app for. Stay length and sort order stay as buttons — they are short lists you change often and want to compare at a glance.
...
- On a wide screen the sidebar's settings collapse behind the same dropdown the phone uses. Open, they had pushed the ranked list — the reason the sidebar exists — down to about two visible rows; closed, eleven options fit.
...
- The downtown meter district no longer carries a price label. A label sits at one point and reads as a claim about that point, and the district's centre is a building on Liberty — not somewhere you can park. Its outline still draws and tapping it still explains the rules.
