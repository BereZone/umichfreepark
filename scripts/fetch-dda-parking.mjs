...
/**
 * PCI's marker title -> the city's LotName, with the id UMichFreePark will use.
 *
...
async function getJson(url, label) {
  const res = await fetch(url, { headers: { 'User-Agent': 'umichfreepark-dda-parking/1.0' } });
...
async function main() {
  ...
  // out even for lots UMichFreePark models from elsewhere, because it settles questions
